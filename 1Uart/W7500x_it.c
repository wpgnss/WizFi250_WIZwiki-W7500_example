...
void UART0_Handler(void)
{
				...
				if(UART_GetITStatus(UART0, UART_IT_FLAG_RXI)) {			
					UART_ClearITPendingBit(UART0, UART_IT_FLAG_RXI);
									...
									if( (u0rx_wr > u0rx_rd && u0rx_wr-u0rx_rd >= U0RX_BUF_SIZE-1) ||(u0rx_wr < u0rx_rd && u0rx_rd == u0rx_wr+1) )	// Buffer Overflow
									{
													UART_SendData(UART0, (uint8_t)'@');
													return;
									}
									u0rx_buf[u0rx_wr] = (uint8_t)UART_ReceiveData(UART0);
							
									if(u0rx_wr < U0RX_BUF_SIZE-1)
												u0rx_wr++;
									else										
												u0rx_wr = 0;
		}
